		...
		<div className="bg-white border-2 border-gray-500 rounded-xl shadow-md overflow-hidden w-50">
			...
			<div className="p-4">
				...
				<div className="flex justify-between items-center">
					<div>
						<p className="text-orange-500 font-bold">{formatMoney(product.price)}</p>
						{product.originalPrice && (
							<p className="text-gray-400 line-through text-sm">{formatMoney(product.originalPrice)}</p>
						)}
					</div>
					<div className="flex items-center text-yellow-500">
						{[...Array(5)].map((_, index) => (
							<Star key={index} fill={index < Math.floor(product.rating) ? "currentColor" : "none"} size={16} />
						))}
					</div>
				</div>
			</div>
...
const MainContent = () => {
	...
	const bestSellingProducts = useMemo(() => [...products].sort((a, b) => b.sales - a.sales).slice(0, 5), [products]);
	...
	return (
		<main className="py-8 px-20 mt-20">
			...
			<div className="relative bg-cover bg-center bg-no-repeat h-[450px] rounded-xl shadow-lg p-8 flex flex-col justify-center items-center bg-[url('../../../Banner1.jpg')] backdrop-blur-lg text-white">
				<h2 className="text-6xl font-bold text-orange-400 text-shadow-md">Welcome to the world of technology!</h2>
				<p className="text-2xl mt-5 text-shadow-md">Never Stop Innovation, Never Stop Progress</p>
			</div>
			...
			<div className="flex justify-center items-center py-4 gap-6">
				<button
					...
					className={`px-6 py-3 text-xl font-semibold rounded-full ${
						selectedFilter === "all" ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-800"
					}`}
				>
					All Categories
				</button>
				...
				{categories.map((category) => (
					<button
						...
						className={`px-6 py-3 text-xl font-semibold rounded-full ${
							selectedFilter === category.type ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-800"
						}`}
					>
						{category.type}
					</button>
				...
				<>
					...
					{categories.map((category) => (
						<div key={category._id} className="mt-10">
							...
							{renderProductGrid(products.filter((product) => product.category === category.type))}
						</div>
					))}
				</>
			) : (
